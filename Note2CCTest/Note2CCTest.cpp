...
int main(int argc, const char * argv[])
{
    ...
    AudioComponentDescription midiProcessorDesc{ kAudioUnitType_MIDIProcessor, Note2CC_COMP_SUBTYPE, kAudioUnitManufacturer_Apple, 0, 0};
    
    try {
        AudioComponent midiProcessorComp = AudioComponentFindNext(nullptr, &midiProcessorDesc);
        XThrowIf(midiProcessorComp == nullptr, -1, "Couldn't find Note2CC. Did you build the Note2CC target?");
        XThrowIfError(AudioComponentInstanceNew(midiProcessorComp, &gMIDIProcessorUnit), "AudioComponentInstanceNew: Note2CC");
        XThrowIfError(AudioUnitInitialize(gMIDIProcessorUnit), "AudioUnitInitialize: Note2CC");
